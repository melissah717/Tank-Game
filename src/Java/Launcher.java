...
import Java.view.Menus.EndGamePanel;
import Java.view.Menus.StartMenuPanel;
...
public class Launcher {
    ...
    public Launcher() {
        this.jFrame = new JFrame();             // creating a new JFrame object
        this.jFrame.setTitle("Tank Wars Game"); // setting the title of the JFrame window.
        this.jFrame.setLocationRelativeTo(null); // this will open the window in the center of the screen.
        this.jFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // when the GUI is closed, this will also shutdown the VM
    }
    ...
    private void initUIComponents() {
        ...
        cardLayout = new CardLayout(); // creating a new CardLayout Panel
        this.mainPanel.setLayout(cardLayout); // set the layout of the main panel to our card layout
        ...
        this.jFrame.add(mainPanel); // add the main panel to the JFrame
        this.jFrame.setResizable(false); //make the JFrame not resizable
    ...
    public void setFrame(String type) {
        ...
        switch (type) {
            case "start" -> this.jFrame.setSize(GameConstants.START_MENU_SCREEN_WIDTH, GameConstants.START_MENU_SCREEN_HEIGHT);
            case "game" -> {
                this.jFrame.setSize(GameConstants.GAME_SCREEN_WIDTH + 18, GameConstants.GAME_SCREEN_HEIGHT + 100);
                (new Thread(this.gamePanel)).start();
            }
            case "end" -> this.jFrame.setSize(GameConstants.END_MENU_SCREEN_WIDTH, GameConstants.END_MENU_SCREEN_HEIGHT);
        }
        this.cardLayout.show(mainPanel, type); // change current panel shown on main panel tp the panel denoted by type.
        this.jFrame.setVisible(true); // show the JFrame
    }
